...
-- Each poll costs ~2.2s of subprocess spawning to find and query the language
-- server. The quota it reports moves no faster than Claude's, so a minute-level
-- cycle was burning CPU for nothing. Countdowns tick locally every second.
FETCH_EVERY = 300
...
function Initialize()
    snapshotPath = SKIN:MakePathAbsolute(SKIN:GetVariable("@") .. "Antigravity\\snapshot.json")
    sessionResetUnix = 0
    weeklyResetUnix = 0
...
local function extract_null(raw, key)
    return raw:match('"' .. key .. '"%s*:%s*null')
end
...
function UpdateHealth(raw)
    ...
    -- number on screen is still fresh. A 401/offline is not: hiding it is
...
function Apply()
    ...
    if ok == "true" then
        ...
        if extract_null(raw, "session_used") then
            session = nil
        end
        if extract_null(raw, "weekly_used") then
            weekly = nil
        end
